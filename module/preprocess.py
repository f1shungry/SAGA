...
def is_sparse_coo(tensor):
    return isinstance(tensor, torch.sparse.Tensor) and tensor.layout == torch.sparse_coo

def slice_sparse_matrix_with_mask(mask, adj, device):

    """
        从稀疏矩阵中提取行和列都在 mask 中标记为 1 的切片。

        参数:
        adjs_s (torch.sparse.Tensor): 输入的稀疏矩阵。
        mask (torch.Tensor): 掩码张量，标记需要提取的行和列。

        返回:
        torch.sparse.Tensor: 提取出的稀疏切片。
        """
    # 获取 train_mask 中标记为 1 的索引
    original_indices = torch.where(mask)[0].to(device)  # 这些是原始的索引

    # 提取稀疏矩阵的 indices 和 values
    row_indices, col_indices = adj.indices()
    values = adj.values()

    # 找到需要保留的行和列
    keep_mask = torch.isin(row_indices, original_indices) & torch.isin(col_indices, original_indices)

    # 根据 keep_mask 提取出有效的行和列
    filtered_row_indices = row_indices[keep_mask]
    filtered_col_indices = col_indices[keep_mask]
    filtered_values = values[keep_mask]

    # 构造新的索引映射
    new_indices = {old_index.item(): new_index for new_index, old_index in enumerate(original_indices)}

    # 重新映射 row 和 col
    new_row_indices = torch.tensor([new_indices[idx.item()] for idx in filtered_row_indices]).to(device)
    new_col_indices = torch.tensor([new_indices[idx.item()] for idx in filtered_col_indices]).to(device)

    # 使用提取的有效索引和值构建新的稀疏矩阵
    sliced_adj = torch.sparse_coo_tensor(
        indices=torch.stack([new_row_indices, new_col_indices]),
        values=filtered_values.to(torch.float32),
        size=(len(original_indices), len(original_indices))  # 新的稀疏矩阵将是 len(original_indices) x len(original_indices)
    ).coalesce()
